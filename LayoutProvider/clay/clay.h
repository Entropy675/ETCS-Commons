...
/*
 * ETCS LOCAL CHANGE -- the guard below accepts 201703L, upstream says 202002L.
 *
 * The only thing in Clay that needs C++20 is the declarative CLAY({...}) macro,
 * which relies on designated initializers. We do not use it: the element tree is
 * built through Clay__OpenElement / Clay__ConfigureOpenElement / Clay__CloseElement
 * with plain struct assignment (RenderProvider/ClayLayout.h), so the implementation
 * itself compiles under C++17 with zero diagnostics -- verified.
 *
 * Bumping the MODULE to C++20 was the alternative and is worse: one provider would
 * compile the shared core headers under a different language standard from every
 * other, which is exactly the kind of silent layout skew this codebase already
 * keeps a manifest check for.
 *
 * THIS IS THE ONLY EDIT TO VENDORED CLAY. On upgrade, re-apply just this.
 */
#if !( \
...
#error "Clay requires C99, C++17 (see the ETCS note above), or MSVC"
#endif

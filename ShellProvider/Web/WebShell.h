...
// ---------------------------------------------------------------------------
// WebShell — an ETCS control thread you can hand scripts to.
//
...
// -- stdio-backed prompts under emscripten, which "terminal" the browser is -- and that had
...
// ---------------------------------------------------------------------------
class WebShell : public ThreadBase<WebShell>,
                   public DeletableBase<WebShell>,
                   public LifecycleBase<WebShell>
{
public:
    WIRE_TYPE_IDENTITY(WebShell);
    ...
    {
        WebShell* child = this->addTag<WebShell>();
        if (!child) return 0;
